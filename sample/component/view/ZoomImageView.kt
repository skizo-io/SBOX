...
/*
 frag_book_imageview.setTabListener(object : jp.comico.ui.common.view.ZoomImageView.TapListener {
                override fun onSingleTapConfirmed(e: MotionEvent) {
                    val width = frag_book_imageview?.width?.toFloat()
                    val height = frag_book_imageview?.height?.toFloat()
                    if (height != null && width != null) {
                        sendDirection(e, width, height)
                    }
                }

                override fun onDoubleTapConfirmed(e: MotionEvent): Boolean {
                    dataListener?.also {
                        val width = frag_book_imageview?.width?.toFloat()
                        val height = frag_book_imageview?.height?.toFloat()
                        if(width == null || height == null) return false
                        if (isCenterClick(e, width, height, it.getIsVerticalMode())) {
                            return true
                        }
                        sendDirection(e, width, height)
                    }
...
class ZoomImageView : AppCompatImageView {
    ...
    private var mXY: FloatArray? = null

    private var mfixX = -1f
    private var mfixY = -1f
    ...
    var inOutsideX = false
        private set
    var inOutsideY = false
        private set
    ...
    val zoomedRect: RectF
        get() {
            if (mScaleType == ImageView.ScaleType.FIT_XY) {
                throw UnsupportedOperationException("getZoomedRect() not supported with FIT_XY") as Throwable
            }
    ...
    internal var mTapListener: TapListener? = null
    ...
    private fun savePreviousImageValues() {
        if (matrix != null && viewHeight != 0 && viewWidth != 0) {
            matrix!!.getValues(mXY)
            prevMatrix!!.setValues(mXY)
    ...
    public override fun onSaveInstanceState(): Parcelable? {
        ...
        matrix!!.getValues(mXY)
        bundle.putFloatArray("matrix", mXY)
    ...
    public override fun onRestoreInstanceState(state: Parcelable) {
        if (state is Bundle) {
            ...
            mXY = bundle.getFloatArray("matrix")
            prevMatrix!!.setValues(mXY)
    ...
    fun setZoom(
        ...
        //
        if (!onDrawReady) {
            scaleType?.let { delayedZoomVariables = ZoomVariables(scale, focusX, focusY, it) }
            return
        }
        ...
        matrix!!.getValues(mXY)
        mXY?.set(Matrix.MTRANS_X, -(focusX * imageWidth - viewWidth * 0.5f))
        mXY?.set(Matrix.MTRANS_Y, -(focusY * imageHeight - viewHeight * 0.5f))
        matrix!!.setValues(mXY)
        fixTrans()
        imageMatrix = matrix
    }
    ...
    private fun fixTrans() {
        matrix!!.getValues(mXY)
        val transX = mXY!![Matrix.MTRANS_X]
        val transY = mXY!![Matrix.MTRANS_Y]
        ...
        if (transX >= 0) {
            ...
            if (transX <= viewWidth - imageWidth + 1) {
                inOutsideX = true
            } else {
                inOutsideX = false
            }
        ...
        if (transY >= 0) {
            ...
            if (transY <= viewHeight - imageHeight + 1) {
                inOutsideY = true
            } else {
                inOutsideY = false
            }
    ...
    private fun fixScaleTrans() {
        ...
        if (imageWidth < viewWidth) {
            mXY?.set(Matrix.MTRANS_X, (viewWidth - imageWidth) / 2)
        }

        if (imageHeight < viewHeight) {
            mXY?.set(Matrix.MTRANS_Y, (viewHeight - imageHeight) / 2)
        }
        matrix!!.setValues(mXY)
    }
    ...
    private fun fitImageToView() {
        ...
        if (!isZoomed && !imageRenderedAtLeastOnce) {
            ...
            prevMatrix!!.getValues(mXY)

            //
            // Rescale Matrix after rotation
            //
            mXY?.set(Matrix.MSCALE_X, matchViewWidth / drawableWidth * currentZoom)
            mXY?.set(Matrix.MSCALE_Y, matchViewHeight / drawableHeight * currentZoom)
    ...
    private fun translateMatrixAfterRotate(
        ...
        if (imageSize < viewSize) {
            ...
            //
            mXY?.set(axis, (viewSize - drawableSize * mXY!![Matrix.MSCALE_X]) * 0.5f)

        } else if (trans > 0) {
            //
            // The image is larger than the view, but was not before rotation. Center it.
            //
            mXY?.set(axis, -((imageSize - viewSize) * 0.5f))
            ...
            mXY?.set(axis, -(percentage * imageSize - viewSize * 0.5f))
        }
    }
    ...
    fun setTabListener(listener: TapListener) {
    ...
    private inner class GestureListener : GestureDetector.SimpleOnGestureListener() {
        ...
        override fun onDoubleTap(e: MotionEvent): Boolean {
            if (mTapListener != null && !mTapListener!!.onDoubleTapConfirmed(e)) return false
    ...
    private fun scaleImage(
        ...
        currentZoom = currentZoom * deltaScale.toFloat()
    ...
    private inner class DoubleTapZoom internal constructor(
        ...
        private fun interpolate(): Float {
            ...
            var elapsed = (currTime - startTime) /  200f
    ...
    private fun transformCoordBitmapToTouch(bx: Float, by: Float): PointF? {
        if (drawable != null) {
            ...
            val px = bx / origW
            val py = by / origH
            val finalX = mXY!![Matrix.MTRANS_X] + imageWidth * px
            val finalY = mXY!![Matrix.MTRANS_Y] + imageHeight * py
            return PointF(finalX, finalY)
        }
        return null
    }
    ...
    private inner class Fling internal constructor(velocityX: Int, velocityY: Int) : Runnable {
        ...
        init {
            ...
            mContext?.let { scroller = CompatScroller(it) }
            matrix!!.getValues(mXY)

            val startX = mXY!![Matrix.MTRANS_X].toInt()
            val startY = mXY!![Matrix.MTRANS_Y].toInt()
            val minX: Int
            val maxX: Int
            val minY: Int
            val maxY: Int
    ...
    companion object {
        ...
        private val SUPER_MAX_MULTIPLIER = 1f
    }
}
/**
